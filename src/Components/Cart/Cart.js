...
const Cart = () => {
  ...
  const showaleart = () => {
    Swal.fire({
      icon: "error",
      title: "Oops...",
      text: "payment stystem on development mood, sorry for short time not able to add. other wise you able to payment use you card",
  ...
  return (
    <div className="modal-cart">
      ...
      <div className="full-cart">
        <div>
          {!cart.length ? (
            <h4 className="text-center mt-5">Cart Empty</h4>
          ) : (
            cart?.map((product) => (
              <>
                <Row className="m-0 border-bottom" key={product.id}>
                  ...
                  >
                    ...
                    <button
                      ...
                      className="text-decoration-underline d-inline bg-transparent border-0 p-0 m-0"
                    >
                  ...
                  {/* cart item text  */}
                  <Col lg={8} md={8} sm={8} xs={8} className="row m-0 p-3">
        ...
        {cart.length > 0 && (
          <div className="my-3 checkout-cart">
            <p className="d-flex justify-content-between align-items-center mx-0 px-2 my-2">
              <span className="total">Total:</span> <span>${totalPrice}</span>
            </p>
            <Button
              as={NavLink}
              to="#"
